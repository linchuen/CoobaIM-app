...
import { loadFriends, selectFirendInfoList } from "./ChatPageSlice";
import { selectTokenInfo } from "../common/globalSlice";

const ChatPage: React.FC = () => {
  const dispatch = useAppDispatch()
  const friendInfos = useAppSelector(selectFirendInfoList)
  const token = useAppSelector(selectTokenInfo)

  useEffect(() => {
    if (token !== null) {
      dispatch(loadFriends([token.userId]));
    }
  }, [token, dispatch])
